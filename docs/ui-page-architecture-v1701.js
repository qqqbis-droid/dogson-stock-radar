(()=>{
  if(window.__DOGSON_PAGE_ARCH_V1720__) return;
  window.__DOGSON_PAGE_ARCH_V1720__=true;
  ...
  function entryKey(r,m){try{const mk=m==='close'||!liveIntra()?(typeof closeMarket!=='undefined'?closeMarket:undefined):(typeof intraMarket!=='undefined'?intraMarket:undefined);return entryDecision(r,mk)?.key||''}catch{return''}}
  function countLike(rows,rx){return rows.filter(r=>rx.test(stage(r))).length}

  function swingSummary(rows,m,title){
    const setup=countLike(rows,/蓄勢/),launch=countLike(rows,/剛啟動/),pull=countLike(rows,/回踩/),trend=countLike(rows,/趨勢|持有/),hot=countLike(rows,/過熱/),weak=countLike(rows,/轉弱|失效/);const green=rows.filter(r=>entryKey(r,m)==='green').length;const fallback=m==='intraday'&&!liveIntra();
    return{title:fallback?'最近完整波段候選':title,sub:fallback?'目前不是通過即時品質門檻的盤中狀態，改用最近完整盤後波段資料；5分鐘與盤中資金訊號暫停。':m==='close'?'收盤後用完整日K、60分K、籌碼與位置整理明日觀察名單。':'盤中用市場、族群、波段結構與目前位置找值得追蹤的機會。',items:[['🌱',setup,'蓄勢'],['🔥',launch,'剛啟動'],['🟡',pull,'回踩'],['🚂',trend,'趨勢中'],['🟢',green,m==='close'||fallback?'位置可觀察':'可試單'],['🚫',hot,'過熱'],['⚠️',weak,'轉弱/失效']]};
  }
