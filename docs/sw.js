const CACHE='dogson-free-v1524';

self.addEventListener('install', event => {
  self.skipWaiting();
  event.waitUntil(
    caches.open(CACHE).then(cache => cache.addAll(['./manifest.webmanifest','./hourly.js?v=1524','./realtime-config.js?v=151','./realtime.js?v=151']))
  );
});
